        ...
        <section className="py-24 relative overflow-hidden">
            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                    ...
                    >
                        <form onSubmit={handleSubmit} className="bg-black/60 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-xl shadow-2xl relative group hover:border-cyan-500/20 transition-all duration-500">
                            {/* Animated gradient border glow */}
                            <div className="absolute -inset-[1px] bg-gradient-to-r from-cyan-500/0 via-cyan-500/20 to-cyan-500/0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 -z-10 blur-sm" />
                            {/* CRT Overlay */}
                                ...
                                {isSuccess ? (
                                    ...
                                    >
                                        ...
                                        <div className="text-center">
                                            <h3 className="text-xl font-bold text-white tracking-[0.2em] uppercase mb-2">Transmission_Complete</h3>
                                            <p className="text-gray-500 text-xs tracking-widest">PAYLOAD_RECEIVED // RESPONSE_QUEUED</p>
                                        </div>
                                    </motion.div>
                                ) : (
                                    <>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            <TerminalInput label="USER_ID" placeholder="Enter identification..." required />
                                            <TerminalInput label="RETURN_ADDR" type="email" placeholder="Enter email..." required />
                                        </div>
                                        ...
                                        <button 
                                            ...
                                            type="submit" 
                                            className={`w-full py-5 font-bold rounded-lg transition-all flex items-center justify-center gap-3 group/btn relative overflow-hidden border ${
                                                isSubmitting ? 'bg-cyan-900/30 border-cyan-500/30 text-cyan-500 cursor-wait' : 'bg-cyan-500/10 border-cyan-500/50 hover:border-cyan-400 text-cyan-400 hover:bg-cyan-500/20 shadow-[0_0_20px_rgba(6,182,212,0.1)] hover:shadow-[0_0_30px_rgba(6,182,212,0.25)]'
                                            }`}
                                        >
                                            {isSubmitting ? (
                                                <>
...
const TerminalInput = ({ label, type = "text", placeholder, required }: { label: string, type?: string, placeholder: string, required?: boolean }) => (
    <div className="space-y-3">
